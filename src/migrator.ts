 ...
 *
 * There is a fourth reason specific to this service, and it is the strongest one. The four
 * constraints in `migrations.ts` — the ones that make the event store refuse a raw subject, refuse
 * a person's event without a pseudonym, refuse a free-text property and refuse a faked erasure —
 * are the whole privacy argument. If the service could create them at boot, it could also start
 * before them, and there would be a window in which the guarantees this repository is built on
 * were unenforced. `index.ts` asserts the version instead and refuses to serve below it.
 *
...
try {
  const result = await migrate(sql as unknown as Sql, MIGRATIONS, {
    service: SERVICE,
    // Zero. This service has never existed, so there is no hand-built schema to adopt — and a
    // non-zero baseline would record the constraint migrations as applied without running them.
