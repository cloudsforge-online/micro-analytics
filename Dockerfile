...
# frozen. `@cloudsforge/contracts-events` is not optional here — it owns `verifyDelivery`, which is
# what makes the inbox refuse a body that was altered between a producer's outbox and this handler.
...
# ══════════════════════════════════════════════════════════════════════════════════════════════
# NO SECRET IS BAKED IN, AND FOR THIS SERVICE THAT SENTENCE HAS TEETH.
#
# There is no ENV line here on purpose — least of all ANALYTICS_PSEUDONYM_KEY. Every other secret
# in the estate answers "can an attacker act as us". The pepper answers "was the pseudonymisation
# ever real": with it and a candidate user id, anyone can compute a lookup key and learn whether
# that person is in the store, and while their salt exists, recover their behavioural history. An
# image layer is copied to every registry mirror that ever pulled the tag, and a layer cannot be
# rotated. `.dockerignore` excludes `.env`, `.env.*` and `.git` for the same reason.
# ══════════════════════════════════════════════════════════════════════════════════════════════
...
# below SCHEMA_VERSION the four constraints this service's privacy properties rest on
# (`events_subject_shape`, `events_person_has_pseudonym`, `events_props_allowed`,
# `subject_keys_erased`) may not exist, and a service that could create them at boot is a service
